...
def get_toc_json_from_pdf(pdf_file):
    ...
    json_response =json.loads( json.dumps(toc_json, indent=4))
    return toc_json
...
class PDFProcessingRequest(BaseModel):
    pdf_file_path: str = os.path.expanduser("~/DocAppBackend/files/rooks 9th edition.pdf")  # Default file path
...
async def process_pdf_chapters(request: PDFProcessingRequest):
    ...
    """
    pdf_file = request.pdf_file_path
    ...
    add_or_update_file(BOOK_NAME, expanded_chapters, chapter_names)
    ...
    # Load TOC JSON
    toc_json = get_toc_json_from_pdf(pdf_file)
    if not toc_json:
    ...
    for entry in chapters:
        ...
        try:
            response = process_text_and_index(chapter_text_md, source_id=chapter_title, file_name=BOOK_NAME)
            if response:
